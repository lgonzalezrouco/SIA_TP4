...
from oja.plots import (plot_convergence, plot_loadings_comparison,
                       plot_projections_comparison, plot_sweep_results,
                       plot_table)
...
class OjaExperiment:
    ...
    def run(self):
        ...
        print(f"{'Feature':<15} | {'sklearn PCA':<12} | {'Oja Rule':<12} | {'Diff Absoluta':<12}")
        print("-" * 58)
        for i, feature in enumerate(feature_names):
            diff = abs(pca_loadings[i] - w_oja_aligned[i])
            print(f"{feature:<15} | {pca_loadings[i]:>12.6f} | {w_oja_aligned[i]:>12.6f} | {diff:>12.6f}")
        ...
        plot_loadings_comparison(feature_names, pca_loadings, w_oja_aligned, output_dir=self.output_dir)
        plot_table(feature_names, pca_loadings, w_oja_aligned, output_dir=self.output_dir)
        plot_projections_comparison(countries, X_std, pca_loadings, w_oja_aligned, output_dir=self.output_dir)
