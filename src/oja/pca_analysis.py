...
class PCAAnalyzer:
    ...
    def plot_boxplots(self):
        ...
        axes[0].set_title("Distribución de Variables Originales (Escalas muy distintas)")
        axes[0].tick_params(axis='x', rotation=45)
        axes[0].set_yscale('log') # Use log scale for better visualization due to Area/GDP differences
    ...
    def plot_scree(self):
        ...
        plt.plot(x, cumulative_variance, marker='o', color='red', label='Varianza Explicada Acumulada')
    ...
    def plot_loadings(self):
        ...
        sns.barplot(x=self.feature_names, y=loadings, hue=self.feature_names, legend=False, palette='viridis')
    ...
    def plot_country_ranking(self):
        ...
        sns.barplot(x='PC1_Score', y='Country', data=df_ranking, hue='Country', legend=False, palette='coolwarm')
    ...
    def plot_biplot(self):
        ...
        for i, country in enumerate(self.countries):
            plt.annotate(country, (pc1[i], pc2[i]), xytext=(5,0), textcoords='offset points', fontsize=9)
            
        # Plot loading vectors
        for i, feature in enumerate(self.feature_names):
            plt.arrow(0, 0, loadings[i, 0]*4, loadings[i, 1]*4, 
                      color='r', alpha=0.8, head_width=0.1, head_length=0.1)
            plt.text(loadings[i, 0]*4.5, loadings[i, 1]*4.5, feature, color='r', ha='center', va='center', fontsize=11, fontweight='bold')
    ...
    def save_csv_results(self):
        ...
        df_loadings = pd.DataFrame({
            ...
            'PC1_Loading': self.pca.components_[0]
        })
        df_loadings.to_csv(os.path.join(self.output_dir, "pc1_loadings.csv"), index=False)
        ...
        df_ranking.to_csv(os.path.join(self.output_dir, "country_pca_scores.csv"), index=False)
