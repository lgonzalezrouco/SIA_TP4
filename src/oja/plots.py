...
def plot_loadings_comparison(feature_names, pca_loadings, oja_loadings, output_dir="results/oja"):
    os.makedirs(output_dir, exist_ok=True)
...
def plot_sweep_results(df_results, output_dir="results/oja"):
    ...
    sns.barplot(data=df_inverse, x="Method", y="Angle_Degrees", hue="Learning_Rate", palette="viridis", errorbar=None)
    ...
    sns.barplot(data=df_zscore, x="Decay_Type", y="Angle_Degrees", hue="Learning_Rate", palette="mako", errorbar=None)
    ...
    sns.barplot(data=df_lr_005, x="Method", y="Angle_Degrees", hue="Decay_Type", palette="flare", errorbar=None)
...
def plot_projections_comparison(countries, X_std, pca_loadings, oja_loadings, output_dir="results/oja"):
    os.makedirs(output_dir, exist_ok=True)
    ...
    plt.plot([min_val, max_val], [min_val, max_val], 'r--', label='Identidad (PCA = Oja)', alpha=0.7)
    
    # Anotar los países
    for i, country in enumerate(countries):
        plt.annotate(country, (pca_proj[i], oja_proj[i]), xytext=(5, 5), textcoords='offset points', fontsize=9, alpha=0.8)
